 ...
 * @since 1.0
 */
public class TailoredCopyProcessAction extends CopyAction {

	private ProcessComponent processToCopy;
	/**
	 * @param process 
	 * @see org.eclipse.emf.edit.ui.action.CommandActionHandler#run()
	 */
	public void run(ProcessComponent process) {
		
		if(updateSelection(process)){
			if (command == null)
				return;
			else {
	...
	public void setToCopy(ProcessComponent processToCopy){
		this.processToCopy = processToCopy;
	}
	
	/**
	 * @see org.eclipse.emf.edit.ui.action.CommandActionHandler#updateSelection(org.eclipse.jface.viewers.IStructuredSelection)
	 */
	public boolean updateSelection(ProcessComponent selection) {
