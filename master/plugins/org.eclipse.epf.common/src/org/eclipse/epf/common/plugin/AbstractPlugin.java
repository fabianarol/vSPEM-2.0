...
public abstract class AbstractPlugin extends AbstractUIPlugin {
	...
	/**
	 * Returns the shared file Html given the relative path.
	 * <p>
	 * Note: The returned image will be automatically freed when the plug-in
	 * shuts down.
	 * 
	 ...
	 * @return The html.
	 */
	public File getSharedHtml(String relativePath) {
		try {
			String path = getLocalizedFile(relativePath, true);
	...
	public File getSharedJasper(String relativePath) {
		try {
			String path = getLocalizedFile(relativePath, true);
			File file = new File(path);
			return file;
	...
	public File getSharedGif(String relativePath) {
		try {
			String path = getLocalizedFile(relativePath, true);
			File file = new File(path);
			return file;
