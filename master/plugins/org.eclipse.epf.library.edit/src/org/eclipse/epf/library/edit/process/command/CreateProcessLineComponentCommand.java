...
import org.eclipse.epf.library.edit.navigator.ProcessLinesPackageItemProvider;
...
public class CreateProcessLineComponentCommand extends CreateChildCommand {
	 ...
	 */
	public CreateProcessLineComponentCommand(EditingDomain domain, EObject owner,
	...
	public void execute() {
		...
		for (Iterator iter = configs.iterator(); iter.hasNext();) {
			...
			//Si no es instancia de ProcessFamily, nuestras lineas de proceso no van a pertenecer a ninguna familia - Añado nuestro objeto a la configuracion
			if (!(element instanceof ProcessFamily)) {
				methodConfigs.add(element);
			}
		}
		//Si no tenemos ningún método de configuración error - Aquí tuve una incidencia, pero se resolvio añadiendo el elemento a la configuracion
		...
		ProcessLinesPackageItemProvider adapter = (ProcessLinesPackageItemProvider) helper;

		ArrayList procClasses = new ArrayList();
		//Plantilla?Nuestras lineas de procesos no tienen plantillas de momento
...
//		}
		...
		final ProcessLineComponent procComp = (ProcessLineComponent) child;
		EClass procType = adapter.getProcessType();//LineProcess
		procComp.setProcessLine((Process) UmaFactory.eINSTANCE.create(procType));
...
//		if(procComp.getProcessLine() instanceof ProcessPlanningTemplate) {
...
//		}
		...
		String msg = NLS.bind(LibraryEditResources.CreateProcessLineComponentCommand_Message, 
				TngUtil.getTypeText(procType.getName()).toLowerCase());
		
		// request input - Rellena la ventana de creación con información
		if (uiHandler.requestInput(LibraryEditResources.newProcessLineComponentDialog_title, 
				msg, userInputs)) {
			String name = (String) nameInput.getInput();
			procComp.setName(name);
			process = procComp.getProcessLine();
		...
		if (canExecute) {
			...
			// is updatable
			//
			status = UserInteractionHelper.checkModify(procComp.getProcessLine()
			...
			Runnable runnable = new Runnable() {
				public void run() {
					// save the resource of parent ProcessLinesPackage
	...
	public void redo() {
		super.redo();
		ProcessLineComponent procComp = (ProcessLineComponent) child;
